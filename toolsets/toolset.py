        ...
        async def call_tool(
            ...
            if name == "Search Deferred Tools":
                ...
                results = await run_sync(
                    lambda: self._search_deferred_tools(query, top_k)
                )
                import json
            ...
            if name == "Call Deferred Tool":
                ...
                if tool_name not in self._deferred_tool_to_element:
                    raise ValueError(f"Deferred tool '{tool_name}' not found")

                element = self._deferred_tool_to_element[tool_name]
                result = await run_sync(
                    lambda: element.execute_tool(tool_name, parameters)
                )
            ...
            if name in self._deferred_tool_to_element:
                raise ValueError(
